      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
                ...
                <div className="bg-gray-50/50 p-6 rounded-[2rem] border-2 border-dashed border-gray-100 relative group transition-all hover:border-green-200">
                  ...
                  {uploadedUrl && (
                    <motion.div 
                      ...
                      className="mt-4 p-3 bg-green-50 rounded-xl border border-green-100 flex items-center justify-between"
                    >
                      <div className="flex items-center gap-3">
                        <img src={uploadedUrl} className="w-10 h-10 rounded-lg object-cover border border-green-200" alt="Uploaded" />
                        <span className="text-[10px] font-black text-green-700 uppercase">Local Asset Synced</span>
                      </div>
                      <button 
                        type="button"
                        onClick={() => setUploadedUrl(null)}
                        className="text-[10px] font-black text-green-600 hover:text-green-800 underline"
                      >
                        REPLACE
                      </button>
                    </motion.div>
            ...
            >
              {status === 'submitting' ? 'Transmitting Data...' : 'Submit Hardware Protocol'}
            </button>
          </form>
        )}
      </div>
